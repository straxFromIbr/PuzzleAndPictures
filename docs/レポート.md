...
## 感想
以前簡易的なAnbdroidアプリケーションを作成したときは個人開発であり、MainActivityにほとんどの処理を書くような乱暴な設計でもさほど問題にならなかった。しかし複数人で分担して開発するときには、アプリケーションを構成する機能を分割し、それぞれが過度に依存しすぎないような設計をする必要があると実感した。今回のアプリケーションはドメインロジックは非常に簡易的であった反面、孫コンポーネントまでバケツリレーのようにメソッドを引数として渡していくなど、Viewの部分が複雑になってしまった。
